...
namespace raywtk 
{
    class ListNodeWidget : public IScreenElement
    {
        bool held = false;
    ...
    public:
        ...
        std::unique_ptr<ListNodeWidget> next = nullptr; // Pointer to next node

        ListNodeWidget(int val, std::shared_ptr<raylib::Font> font) : value(val), nodefont(font)  
        {
            content = [this]()
            {
